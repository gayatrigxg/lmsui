...
struct LoanSummaryCardView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(alignment: .bottom) {
                ...
                VStack(alignment: .trailing, spacing: 4) {
                    Text("Total Loan")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                    Text("₹\(loan.totalAmount, specifier: "%.0f")")
                        .font(.title3).bold()
            ...
            HStack {
                Text("\(Int(loan.repaidFraction * 100))% repaid")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
                Spacer()
                Text("\(loan.remainingEMIs) EMIs left")
                    .font(.subheadline)
                    .foregroundColor(.secondary)
            }
        }
...
struct NextEMIBannerView: View {
    ...
    var body: some View {
        HStack(spacing: 8) { // Tighter spacing between elements

            // Icon
            Circle()
                .fill(emi.isUrgent ? Color.alertRed.opacity(0.12) : Color.mainBlue.opacity(0.10))
                .frame(width: 44, height: 44) // Reduced icon size
                .overlay(
                    Image(systemName: emi.isUrgent ? "exclamationmark.circle.fill" : "calendar.badge.clock")
                        .font(.body)
                        .foregroundColor(emi.isUrgent ? .alertRed : .mainBlue)
                )

            VStack(alignment: .leading, spacing: 2) {
                Text("Next EMI") // Shortened to fit better
                    .font(.caption)
                    .foregroundColor(.secondary)
                    .lineLimit(1)
                Text(emi.dueDate)
                    .font(.subheadline).bold() // Scaled down from title3/body
                    .foregroundColor(emi.isUrgent ? .alertRed : .primary)
                    .lineLimit(1)
                    .minimumScaleFactor(0.8) // Allows text to shrink safely
            }

            Spacer(minLength: 4)

            VStack(alignment: .trailing, spacing: 2) {
                // Formatting added for commas (e.g., ₹14,200)
                Text("₹\(emi.amount.formatted(.number.grouping(.automatic)))")
                    .font(.subheadline).bold() // Scaled down to fit with button
                    .foregroundColor(.mainBlue)
                    .lineLimit(1)
                ...
                Text(emi.daysLeft)
                    .font(.caption2) // Scaled down for contrast
            ...
            Button {
                ...
                Text("Pay Now")
                    .font(.caption).bold() // More compact text
                    .foregroundColor(.white)
        ...
        .padding(14) // Slightly reduced padding to maximize internal space
        .background(Color.white)
        .clipShape(RoundedRectangle(cornerRadius: 16))
        .overlay(
            RoundedRectangle(cornerRadius: 16)
                .stroke(emi.isUrgent ? Color.alertRed.opacity(0.4) : Color.clear, lineWidth: 1.5)
        )
    ...
    var body: some View {
        HStack(spacing: 8) {
            Image(systemName: alert.icon)
                .font(.subheadline)
                .foregroundColor(alert.isWarning ? .alertRed : .mainBlue)
            Text(alert.message)
                .font(.subheadline)
                .foregroundColor(.primary)
                .lineLimit(1)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 24)
                .stroke(
                    alert.isWarning ? Color.alertRed.opacity(0.35) : Color.secondaryBlue.opacity(0.25),
                    lineWidth: 1.2
                )
        )
    }
...
// MARK: - 6. Credibility Score Card
struct CredibilityScoreCardView: View {
    let score: Int

    private var scoreColor: Color {
        switch score {
        case 750...: return Color(hex: "#00C48C")
        case 600..<750: return Color.secondaryBlue
        default: return Color.alertRed
        }
    }
    ...
    private var scoreLabel: String {
        ...
        case 650..<750: return "Good"
        case 500..<650: return "Fair"
        default: return "Poor"
        }
    }
...
class HomeDashboardViewModel: ObservableObject {
    ...
    let activeLoan = LoanSummary(
        totalAmount: 500000,
        outstandingBalance: 312000,
        remainingEMIs: 18
    )
    let nextEMI = NextEMIInfo(
        amount: 14200,
        dueDate: "20 Apr 2026",
    ...
    let alerts: [AlertItem] = [
        ...
        AlertItem(icon: "arrow.down.circle.fill",        message: "₹500 cashback credited", isWarning: false)
    ]
